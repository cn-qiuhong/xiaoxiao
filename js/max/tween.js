...
export function tweenProperty(sprite,                  //Sprite object
...
export function slide(sprite, endX, endY, frames = 60, type = ["smoothstep"], yoyo = false, delayBeforeRepeat = 0) {
  return makeTween([
...
//pathX,pathY是数组
export function slidePath(sprite, pathX, pathY, frames) {
  if (!(pathX instanceof Array)) return slide(sprite, pathX, pathY, frames);
  let index = -1;

  function callback() {
    index++;
    if (index < pathX.length - 1)return slide(sprite, pathX[index], pathY[index], frames).onComplete = callback;
    else return slide(sprite, pathX[index], pathY[index], frames);
  }
...
//变大变小，循环
export function breathe(sprite, endScaleX, endScaleY,
...
//快速变大变小
export function strobe(sprite, scaleFactor = 1.3, startMagnitude = 10, endMagnitude = 20,
...
//抖动
export function wobble(sprite, scaleFactorX = 1.2, scaleFactorY = 1.2, frames = 10, xStartMagnitude = 10, xEndMagnitude = 10,
...
export function followCurve(sprite, pointsArray, totalFrames, type = ["smoothstep"], yoyo = false, delayBeforeRepeat = 0) {
...
export function walkPath(sprite,                   //The sprite
...
export function walkCurve(sprite,                  //The sprite
